...
# Версия 5.3 - Финальная версия с исправлениями
...
# 1. Параллельное выполнение независимых задач очистки (требует PowerShell 7)
# 2. Использование Storage Sense API вместо устаревшего CleanMgr (Windows 11 23H2+)
# 3. Переход на машиночитаемый формат winget при его появлении
# 4. Добавление поддержки UsoClient как fallback для обновлений Windows
...
# Для PowerShell 7 используем правильную настройку кодировки
if ($PSVersionTable.PSVersion.Major -ge 7) {
    $PSNativeCommandEncoding = [System.Text.Encoding]::UTF8
} else {
    chcp 65001 | Out-Null
}
...
function Remove-FolderContent {
    ...
    try {
        ...
        if (-not $resolvedPath) {
            Write-ColoredLog "Путь не существует: $Path" "WARNING"
            return
        }
    } catch {
        Write-ColoredLog "Не удалось разрешить путь: $Path" "WARNING"
        return
    }
    
    # Проверяем против защищенных путей
    foreach ($protected in $protectedPaths) {
        try {
            $protectedResolved = (Resolve-Path -LiteralPath $protected -ErrorAction SilentlyContinue).Path
            if ($protectedResolved -and ($resolvedPath -eq $protectedResolved)) {
    ...
    if (Test-Path $resolvedPath) {
        $sizeBefore = Get-FolderSize -Path $resolvedPath
        try {
            # Получаем элементы для удаления, @() гарантирует, что результат всегда будет массивом
            $items = @(Get-ChildItem -Path $resolvedPath -Force -ErrorAction SilentlyContinue)
            $itemCount = $items.Count
            
            if ($itemCount -gt 0) {
                # Снимаем атрибуты и удаляем
                foreach ($item in $items) {
                    try {
                        # Снимаем атрибуты ReadOnly, Hidden, System
                        if ($item.Attributes -band [System.IO.FileAttributes]::ReadOnly) {
                            $item.Attributes = $item.Attributes -band (-bnot [System.IO.FileAttributes]::ReadOnly)
                        }
                        # Используем длинные пути для Windows
                ...
                $sizeAfter = Get-FolderSize -Path $resolvedPath
...
function New-SystemRestorePoint {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            # В PowerShell 7 используем Windows PowerShell 5.1 для создания точки восстановления
            Write-ColoredLog "Создание точки восстановления через Windows PowerShell..." "INFO"
...
"@
            ...
            $result = & "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe" `
                        -NoLogo -NoProfile -Command $scriptBlock
            ...
            # В Windows PowerShell 5.1 используем нативные командлеты
            Enable-ComputerRestore -Drive "$env:SystemDrive" -ErrorAction SilentlyContinue
            Checkpoint-Computer -Description $Description -RestorePointType "MODIFY_SETTINGS" -ErrorAction Stop
            Write-ColoredLog "Точка восстановления создана: $Description" "SUCCESS"
            return $true
    ...
    try {
        ...
        if (!(Get-Module -ListAvailable -Name PSWindowsUpdate)) {
            ...
            if (-not $nugetProvider -or $nugetProvider.Version -lt "2.8.5.201") {
                ...
                if (-not $?) {
                    throw "Не удалось установить NuGet провайдер. Проверьте подключение к интернету и доступ к PSGallery."
                }
            }
            ...
            # Проверяем успешность установки
            if (-not $? -or !(Get-Module -ListAvailable -Name PSWindowsUpdate)) {
                throw "Не удалось установить модуль PSWindowsUpdate"
            }
        ...
        # Получаем версию модуля для определения доступных параметров
        $moduleVersion = (Get-Module PSWindowsUpdate).Version
        Write-ColoredLog "Загружен PSWindowsUpdate версии $moduleVersion" "INFO"
        
        return $true
    } catch {
        Write-ColoredLog "Не удалось установить/загрузить PSWindowsUpdate: $_" "ERROR"
        Write-ColoredLog "Попробуйте вручную установить модуль: Install-Module PSWindowsUpdate -Force" "INFO"
        return $false
    }
}
...
function Update-WindowsSystem {
    ...
    try {
        ...
        # Сохраняем версию модуля для дальнейшего использования
        $moduleVersion = (Get-Module PSWindowsUpdate).Version
        ...
        if ($availableUpdates.Count -gt 0) {
            ...
            # Используем Install-WindowsUpdate с совместимыми параметрами
            Write-ColoredLog "Запуск процесса установки всех обновлений..." "INFO"
            
            try {
                # В новых версиях PSWindowsUpdate используется -AutoReboot вместо -IgnoreReboot
                if ($moduleVersion.Major -ge 2 -and $moduleVersion.Minor -ge 3) {
                    # Новая версия модуля
                    $installResults = Install-WindowsUpdate -MicrosoftUpdate -AcceptAll -AutoReboot:$false -Verbose
                } else {
                    # Старая версия модуля
                    $installResults = Install-WindowsUpdate -MicrosoftUpdate -AcceptAll -IgnoreReboot -Verbose
                }
                ...
                if ($installResults) {
                    ...
                    # Если результаты не получены, пробуем альтернативный метод
                    Write-ColoredLog "Установка обновлений альтернативным методом..." "INFO"
                    
                    foreach ($update in $availableUpdates) {
                        $progress = [math]::Round((($installedCount + $failedCount + 1) / $availableUpdates.Count) * 100)
                        
                        Write-Host "[$($installedCount + $failedCount + 1)/$($availableUpdates.Count)] " -NoNewline -ForegroundColor Cyan
                        Write-Host "$($update.Title)" -ForegroundColor Gray
                        
                        try {
                            # Используем правильные параметры в зависимости от версии модуля
                            if ($moduleVersion.Major -ge 2 -and $moduleVersion.Minor -ge 3) {
                                Get-WindowsUpdate -KBArticleID $update.KB -Install -AcceptAll -AutoReboot:$false -Verbose
                            } else {
                                Get-WindowsUpdate -KBArticleID $update.KB -Install -AcceptAll -IgnoreReboot -Verbose
                            }
                            ...
                            if ($errorMsg -like "*already installed*" -or $errorMsg -like "*0x80240022*") {
                ...
                # Проверяем, что действительно установилось
                Write-ColoredLog "Проверка установленных обновлений..." "INFO"
                Start-Sleep -Seconds 3 # Даем системе время обновить базу
                
                $remainingUpdates = @()
                try {
                    $remainingUpdates = Get-WindowsUpdate -MicrosoftUpdate -ErrorAction Stop
                } catch {
                    $remainingUpdates = Get-WindowsUpdate -ErrorAction SilentlyContinue
                }
                
                if ($remainingUpdates.Count -lt $availableUpdates.Count) {
                    $actuallyInstalled = $availableUpdates.Count - $remainingUpdates.Count
                    if ($actuallyInstalled -ne $installedCount) {
                        Write-ColoredLog "Фактически установлено: $actuallyInstalled обновлений." "INFO"
                        $script:windowsUpdatesCount = $actuallyInstalled
                    }
                }
            } catch {
                Write-ColoredLog "Критическая ошибка при установке обновлений: $_" "ERROR"
                $script:errorsCount++
            }
...
function Update-Applications {
    ...
    try {
        ...
        # Получаем список обновлений для анализа
        ...
        Write-Host $updateCheckOutput
        
        # Анализируем, есть ли обновления
        # ВНИМАНИЕ: Парсинг текстового вывода winget - временное решение.
        # Winget пока не предоставляет машиночитаемый формат (JSON/XML).
        # Этот код может сломаться при изменении формата вывода winget.
        $hasUpdates = $false
        $updateCount = 0
        ...
        if ($hasUpdates) {
            ...
            # ВАЖНО: Используем Start-Process с -Wait для гарантии выполнения
            $upgradeProcess = Start-Process -FilePath $wingetPath `
            ...
            if ($upgradeProcess.ExitCode -eq 0) {
                ...
                Write-Host ""
                Write-ColoredLog "Обновление завершено с кодом: $($upgradeProcess.ExitCode)" "WARNING"
                
                # Проверяем, что действительно обновилось
                Write-ColoredLog "Проверка результатов обновления..." "INFO"
                $tempFile2 = [System.IO.Path]::GetTempFileName()
                $checkProcess2 = Start-Process -FilePath $wingetPath `
                                             -ArgumentList "upgrade" `
                                             -NoNewWindow `
                                             -RedirectStandardOutput $tempFile2 `
                                             -PassThru -Wait
                
                $afterUpdateOutput = Get-Content $tempFile2 -Raw -Encoding UTF8
                Remove-Item $tempFile2 -Force -ErrorAction SilentlyContinue
                
                $remainingUpdates = 0
                $lines2 = $afterUpdateOutput -split "`n"
                ...
                if ($remainingUpdates -lt $updateCount) {
                    $actuallyUpdated = $updateCount - $remainingUpdates
                    Write-ColoredLog "Успешно обновлено: $actuallyUpdated из $updateCount приложений." "INFO"
                    $script:appUpdatesCount = $actuallyUpdated
                    
                    if ($remainingUpdates -gt 0) {
                        Write-ColoredLog "Не удалось обновить $remainingUpdates приложений (возможно, требуется перезапуск)." "WARNING"
                    }
                }
            }
        } else {
...
function Clear-System {
    ...
    try {
        ...
        if ($clearedCount -gt 0) {
            Write-ColoredLog "Очищено логов: $clearedCount" "SUCCESS"
        }
        if ($errorCount -gt 0) {
            Write-ColoredLog "Не удалось очистить некоторые логи (возможно, используются): $errorCount" "INFO"
        }
    } catch {
        Write-ColoredLog "Ошибка при очистке логов: $_" "WARNING"
    }
    ...
    foreach ($category in $cleanupCategories) {
        ...
        if (Test-Path $categoryPath) {
            # Исправлено: полное имя свойства StateFlags с номером sageset
            Set-ItemProperty -Path $categoryPath -Name "StateFlags$sageset" -Value 2 -Type DWord -Force -ErrorAction SilentlyContinue
        }
    }
    
    # Запускаем cleanmgr с увеличенным таймаутом (10 минут для медленных дисков)
    ...
    if (-not $cleanmgrProcess.HasExited) {
        Write-ColoredLog "Очистка диска занимает больше времени чем ожидалось..." "WARNING"
        
        # Проверяем, действительно ли процесс завис
        $cleanmgrRunning = Get-Process -Name cleanmgr -ErrorAction SilentlyContinue
        if ($cleanmgrRunning) {
            Write-ColoredLog "Процесс cleanmgr все еще активен. Ожидаем завершения..." "INFO"
            # Даем еще 5 минут
            $cleanmgrProcess | Wait-Process -Timeout 300 -ErrorAction SilentlyContinue
        }
        
        if (-not $cleanmgrProcess.HasExited) {
            try {
                $cleanmgrProcess | Stop-Process -Force -ErrorAction SilentlyContinue
                Write-ColoredLog "Процесс очистки принудительно завершен." "WARNING"
            } catch {}
        }
    ...
    # Команда удаляет только ОДНУ самую старую точку восстановления
    Write-ColoredLog "Удаление самой старой точки восстановления..." "INFO"
    try {
        ...
        if (Test-Path $vssadminPath) {
            ...
            if ($vssProcess.ExitCode -eq 0) {
                Write-ColoredLog "Самая старая точка восстановления удалена." "SUCCESS"
            } elseif ($vssProcess.ExitCode -eq 1) {
                Write-ColoredLog "Нет доступных точек восстановления для удаления." "INFO"
            } else {
                Write-ColoredLog "Не удалось удалить точку восстановления (код: $($vssProcess.ExitCode))" "WARNING"
            }
        }
    } catch {
...
# Диагностическая проверка для winget
if ($script:appUpdatesCount -gt 0) {
    Write-Host ""
    Write-ColoredLog "Проверка оставшихся обновлений приложений..." "INFO"
    
    # Быстрая проверка через winget
    ...
    if ($wingetPath) {
        ...
        $checkOutput = Get-Content $tempFile -Raw -Encoding UTF8
        Remove-Item $tempFile -Force -ErrorAction SilentlyContinue
        
        $remainingCount = 0
        $lines = $checkOutput -split "`n"
        foreach ($line in $lines) {
            if ($line -match "^\S+\s+[\d\.\-]+\s+[\d\.\-<>]+\s+\S+") {
                $remainingCount++
            }
        ...
        if ($remainingCount -gt 0) {
            Write-ColoredLog "Внимание: Обнаружено $remainingCount необновленных приложений." "WARNING"
            Write-ColoredLog "Возможно, требуется перезапуск или ручное обновление." "INFO"
        } else {
            Write-ColoredLog "Все приложения успешно обновлены." "SUCCESS"
        }
    }
}
