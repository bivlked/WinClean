...
├── tests/                    # Pester тесты (450 всего; счётчик - прогоном, не грепом)
│   ├── Helpers.Tests.ps1     # Unit-тесты helper-функций (158, дот-сорсят продукт - нужны права админа)
│   ├── Fixes.Tests.ps1       # Валидационные тесты исправлений (182)
...
3. **test** - Pester тесты (450; интеграционные требуют admin - на GitHub runners это выполняется)
...
- `tests/Helpers.Tests.ps1` - 158 unit-тестов (дот-сорсят WinClean.ps1), `tests/Fixes.Tests.ps1` - 182, `tests/Integration.Tests.ps1` - 67 (песочница ФС, требуют admin), `tests/StandHelpers.Tests.ps1` - 17 (без admin), `tests/Docs.Tests.ps1` - 26 (гварды доков, без admin)
...
Invoke-Pester ./tests -Output Detailed              # 450 Pester тестов (считать прогоном, не грепом)
