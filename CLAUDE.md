...
> Текущая версия скрипта: 2.20 (ВЫПУЩЕНА 22.07.2026: GitHub Release с обоими ассетами + PSGallery 2.20 + e2e через опубликованный one-liner)
...
- **PSGallery**: ✅ v2.20 (22.07.2026). Отставание с v2.13 закрыто в 2.19: не публиковались 2.14-2.18, из-за чего встроенная авто-проверка (`Test-ScriptUpdate`) молчала у всех, кто ставил из галереи
...
├── tests/                    # Pester тесты (571 всего; счётчик - прогоном, не грепом)
│   ├── Helpers.Tests.ps1     # Unit-тесты helper-функций (278, дот-сорсят продукт - нужны права админа)
...
3. **test** - Pester тесты (571; интеграционные требуют admin - на GitHub runners это выполняется)
...
- `tests/Helpers.Tests.ps1` - 278 unit-тестов (дот-сорсят WinClean.ps1), `tests/Fixes.Tests.ps1` - 183, `tests/Integration.Tests.ps1` - 67 (песочница ФС, требуют admin), `tests/StandHelpers.Tests.ps1` - 17 (без admin), `tests/Docs.Tests.ps1` - 26 (гварды доков, без admin)
...
### В работе
- [ ] Публикация статьи на Хабре (`docs/_habr/habr-article.md` - текст готов, ждёт скриншоты). ⚠️ Текст писался под старую версию - сверить с v2.20 (появились get.ps1/install.ps1, стенд, ночные прогоны, очистка Driver Store, пофазное выполнение, публичный docs/, `-SkipDiskCleanup`)
- [ ] `MyAI-iodp` (P3): отсутствие winget даёт ERROR и код возврата 1, хотя это состояние машины, а не отказ. НЕ регрессия - поведение с v1.2. Поймано полным e2e без `-SkipUpdates`; обычные прогоны стенда до этой ветки не доходят
