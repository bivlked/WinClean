...
⚠️ **На 20.07.2026 в `main` 11 незапушенных коммитов** - пуш ждёт решения пользователя (пуш релизом не является и пользователям ничего не отдаёт).

Содержимое 2.17, четыре прохода:
...
4. **MyAI-dtx8 группа B** (самое рискованное: `Remove-FolderContent` - один обход дерева вместо трёх-четырёх, `Get-FolderSize`/`Clear-EventLogs` на raw .NET API, пофазный try/catch с `PhasesCompleted`/`PhasesFailed` в JSON, recovery-маркер после hard kill). Каждый шаг - мутационная проверка (отключить/удалить логику, убедиться, что тест падает), не просто "тесты написаны и зелёные".

222 → 274 теста за сессию (84 → 91 / 126 → 127 менялись по ходу, финал 91+126+57).

Незакрытое: `MyAI-g7rc` (п.1, нужен живой стенд), `MyAI-296v`, `MyAI-dtx8` группы C/D (эффективность п.4-9,19,27-28,31 - частично закрыты; п.4 WU-категории и вся секция "проверить на живой системе" нужны стенда; 39 функций без тестов - закрыты только 8 удаляющих + `Get-SupersededDriverCandidate`, остальные открыты). Durable-ловушки: память `powershell-verification-traps` (включая флейковость `Invoke-SmokeTest.ps1`, ~1 из 3 запусков).
