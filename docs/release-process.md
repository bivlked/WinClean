...
10. Clean up. Delete the release branch once it is merged (locally and on the remote) and resolve any open dependency PRs, so that `main` is the only branch left. The repository has "automatically delete head branches" enabled, so a branch merged through a PR removes itself; a branch merged any other way does not. The tag and the release keep the history, so a merged branch carries no information that is not already in `main`.
