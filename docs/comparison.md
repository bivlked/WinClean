...
- **Safety by design.** A System Restore Point is attempted before the maintenance phases (and the run continues with a warning if it cannot be created). A fixed set of system roots is never accepted as a cleanup target, and path protection normalizes short names and `..` traversal. `-ReportOnly` previews the actions without making maintenance changes. The one-command installers are fail-closed and verify a SHA256 hash before running the downloaded script.
