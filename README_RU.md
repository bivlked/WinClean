...
| `-SkipDiskCleanup` | Пропустить только шаг Storage Sense / Disk Cleanup, он бывает самым долгим | `false` |
...
│  ├─ 💾 Storage Sense, затем Очистка диска, если он не доказал  │
│  │     что сработал (до 23 обработчиков)                      │
