...
- Лицензия MIT, без телеметрии, персональные данные не отправляются. Скрипт обращается только к стандартным сервисам обслуживания (Windows Update, winget, PowerShell Gallery). См. **[SECURITY.md](SECURITY.md)** и **[docs/safety.md](docs/safety.md)**.
